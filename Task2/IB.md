...
| 1   | Используется ли многофакторная аутентификация (MFA) для доступа к критическим системам и сервисам?                                       |   Нет    |       Исходя из С4 модели, система не имеет интеграций с системами для реализации второго фактора, например смс или email       |
...
| 8   | Используются ли системы аутентификации пользователей для доступа к системам?                                                             |   Да    |       Исходя из С4 модели, разные работники работают с разными подсистемами. Например Бухгалтер и Аналитик данных       |
| 9   | Созданы ли замкнутые контуры для подсистем, работающих с чувствительными данными?                                                        |       |              |
| 10  | Ограничен ли доступ к системам из внешних источников?                                                                                    |   Да    |      Исходя из С4 модели, трафик в систему проходит через firewall, а чувствительные действия пользователей проходят аутентификацию        |
| 11  | Ведётся ли единый реестр учётных записей?                                                                                               |       |              |
| 12  | Используются ли средства управления учётными записями на базе единого реестра?                                                          |    Нет   |       В блоке проблем и потребностей явно указано что есть несколько точек регистрации клиентов без координации между ними       |
| 13  | Применяются ли средства для отзыва прав доступа?                                                                                        |       |              |
| 14  | Используются ли средства централизованного контроля доступа?                                                                            |   Нет    |      Исходя из С4 модели, разные части системы проводят аутентификацию действия, например аутентификация менеджеров и Бухгалтеров        |
...
| 7   | Ведётся ли учёт персональных данных, и применяются ли специальные процедуры обработки и хранения для различных типов данных?            |   Да    |      В системе хранятся персональные данные пользователей, но нет информации об используемых процедурах обработки и хранения        |
...
| 11  | Используются ли системы или процессы контроля целостности данных?                                                                       |   Нет    |      В блоке проблем и потребностей явно указано что есть несколько точек регистрации клиентов без координации между ними        |
...
| 1   | Используется ли межсетевой экран для ограничения входящего и исходящего трафика?                                                        |    Да   |       Имеется Firewall для фильтрации трафика       |
...
| 5   | Проводится ли регулярное сканирование уязвимостей операционных систем, приложений и сетевых устройств?                                   |   Нет    |       В модели С4 это не отображено, в описании системы не указано       |
